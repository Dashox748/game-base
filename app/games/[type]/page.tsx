...
const getGameList = async (type: string) => {
  const res = await axios({
    url: `https://api.rawg.io/api/games?key=${process.env.API_KEY}&page_size=40&ordering=-${type}`,
    method: "GET",
    headers: {},
  })
    .then((response) => {
...
export default async function gameListWithType({
  ...
  const gameList = await getGameList(type);
        ...
        {gameList?.results?.map((item: any, index: number) => {
          return (
            <div key={index} className={classes.game_grid}>
              <Image
                src={item.background_image ? item.background_image : bioshock}
                alt={`${item.name} thumb`}
                blurDataURL="data:..."
                placeholder="blur"
                fill
